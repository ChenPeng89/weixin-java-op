...
public class WxOpKefuMessageGsonAdapter implements JsonSerializer<WxOpKefuMessage> {

  @Override
  public JsonElement serialize(WxOpKefuMessage message, Type typeOfSrc, JsonSerializationContext context) {
    ...
    if (WxConsts.CUSTOM_MSG_NEWS.equals(message.getMsgType())) {
      ...
      for (WxOpKefuMessage.WxArticle article : message.getArticles()) {
    ...
    if (WxConsts.CUSTOM_MSG_MPNEWS.equals(message.getMsgType())) {
      ...
      json.addProperty("media_id", message.getOpNewsMediaId());
